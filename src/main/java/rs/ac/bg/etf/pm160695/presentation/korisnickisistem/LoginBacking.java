...
public class LoginBacking implements Serializable {
	...
	@PostConstruct
	public void init() {
		FORWARD_REQUEST_URI = (String) FacesContext.getCurrentInstance()
					.getExternalContext().getRequestMap().get(RequestDispatcher.FORWARD_REQUEST_URI);
	}
	...
	public void loginAction() {
		...
		if (request.getUserPrincipal() != null) {
			...
			if (request.getUserPrincipal().getName().equals(username)) {
				...
				try {
					logger.info("Logovanje sa drugim userom na istoj sesiji. Stari user: " + request.getUserPrincipal().getName()
							+ " Novi user: " + username);
					request.logout();
		...
		try {
			...
			FacesContext.getCurrentInstance()
				.getExternalContext().redirect(FORWARD_REQUEST_URI != null ? FORWARD_REQUEST_URI : request.getContextPath());
		} catch (ServletException e) {
			// login exception
